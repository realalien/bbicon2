...
public class GuiStatusMonitor implements Observer {  // no need to make this class multithread!
	...
	private static String new_status;
	...
	public void removeParentMenu(MenuItem childMenu) {
		if (childMenu.getParent() instanceof Menu) {
			logger.debug("...... remove Parent menu, "
					+ childMenu.getParent());
	...
	public void removeFromThreadPool(long threadID) {
		...
		while (iter.hasNext()) {
			...
			if (thread.getId() == threadID) {
				pt2remove.stop(); // ESP: can use thread.destroy() which is deprecated.
				iter.remove();
				logger.debug("threadID:" + threadID + " was killed! ");
				logger.debug("Tasks collection has "
								+ tasks.size()
	...
	private void createAndShowGUI() throws Exception {
		...
		if (!SystemTray.isSupported()) {
			...
			throw new Exception("createAndShowGUI():SystemTray is not supported!");
		}
		popup = new PopupMenu();
		...
		subtasksListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (e.getSource() instanceof MenuItem) {
					subtaskMenuItemLabel = ((MenuItem)e.getSource()).getLabel();
				}
				...
				case 1: // PAUSE_LABEL pressed
					...
					if (e.getActionCommand().equals(PAUSE_LABEL)){  // pause, resume share one menuitem
						//change label
						((MenuItem) e.getSource()).setLabel(RESUME_LABEL);
						// change thread property, get thread_id from father's label
						threadLabel = ((Menu)((MenuItem) e.getSource()).getParent()).getLabel() ;
				...
				case 2: // RESUME_LABEL pressed
					if (e.getActionCommand().equals(RESUME_LABEL)){  // pause, resume share one menuitem
...
//					if (((MenuItem) e.getSource()).getLabel() == PAUSE_LABEL && e.getActionCommand().equals(PAUSE_LABEL)) {
//						//change label
//						((MenuItem) e.getSource()).setLabel(RESUME_LABEL);
//						// change thread property, get thread_id from father's label, or what else methods?
//						
//						threadLabel = ((Menu)((MenuItem) e.getSource()).getParent()).getLabel() ;
//						thread_id = extractThreadIDFromMenuLabel(threadLabel); 
//						pause_thread(thread_id);
...
//						
//						
//					}else if (((MenuItem) e.getSource()).getLabel() == RESUME_LABEL && e.getActionCommand().equals(RESUME_LABEL)) {
//						//change label
//						((MenuItem) e.getSource()).setLabel(PAUSE_LABEL);
...
//					}else if (((MenuItem) e.getSource()).getLabel() == REMOVE_LABEL) {
...
//						}
//					}
					//Q: shall I create a thread status listener as each updating invokes a lot of jobs to do?!
					logger.debug("=====Here we are in the listener of subtasks for MenuItem of different thread!");
				}
			};
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (pressedItemLabel.contains("Monitoring")) {
					...
					if (ip != null) { // TODO:validate IP
						...
						if (thread != null){
							thread.start();
							String threadInfo  = thread.getName() + " id:" + thread.getId() 
								+ " => reaching:" + ip ;
							...
							trayIcon.displayMessage("Failed to create a thread! ",
									"Unable to create a thread with data " + data.toString(), TrayIcon.MessageType.ERROR);
						}
					} else {
						trayIcon.displayMessage("Failed to create a thread! ",
								"IP address is invalid", TrayIcon.MessageType.ERROR);
					}
				} else {
					trayIcon.displayMessage("Failed to create a thread! ",
							"MenuItem pressed does not contain keyword 'Monitoring'", TrayIcon.MessageType.ERROR);
				}
			}
		}
	...
	public void pause_thread(Long thread_id) {
		...
		while (iter.hasNext()) {
			...
			if (thread.getId() == thread_id) {
				pt2remove.suspend(); // ESP: can use thread.destroy(), test if null
									// ok
		...
		logger.debug("!!!!!!! try to suspend threadID:" + thread_id + " was NOT FOUND! ");
	}
	...
	public void resume_thread(Long thread_id) {
		...
		logger.debug("!!!!!!! try to resume threadID:" + thread_id + " was NOT FOUND! ");
	}
...
//	}
	...
	/**
	 * To write tasks to a file for reloading next time
	 * Note: 
	 * * I think just follow GUI info, as those tasks do not care about the logging
	 * */ 
			...
			for(int j=0 ; j<taskMenu.getItemCount(); j++){
				...
				if (item.getLabel().equals(REMEMBER_TARGET_LABEL)){
					if ((((CheckboxMenuItem) item).getState()) == true ){
						logger.debug("ready to write file with info: " + taskMenu.getLabel());
...
//		logger.debug("Reading last saved monitoring target: " + webappTaskRec);
//	
...
//		if (thread2 != null ) {thread2.start();}
	 ...
	 * "add sub-menu and their listeners", thus use factory pattern here,
	 * Q: question about the exposure of instance property!
	 * */
	private Thread produce_task(String data) {
		String info[] = data.split(":::");
		logger.debug("produce_task():Reading last saved monitoring target: " +data.toString());
		...
		if (task != null) {
			...
			logger.debug("produce_task()...OK with a new thread fully armed with menuitems!");
			return thread;
		}else{
			logger.debug("produce_task()...return null rather than a thread instance.");
			return null;
		}
	}
}
